...
tg() {
    ...
    --sendphoto)
        ...
        local PHOTO=$2
        local CAPTION=$3
        curl -s "$API/sendPhoto" -F "chat_id=$CHAT_ID" -F "photo=@\"$PHOTO\"" -F "caption=$CAPTION" | jq .
        ;;
    --replyphoto)
        shift
        local CHAT_ID=$1
        ...
        local PHOTO=$3
        local CAPTION=$4
        curl -s "$API/sendPhoto" -F "chat_id=$CHAT_ID" -F "reply_to_message_id=$MSG_ID" -F "photo=@\"$PHOTO\"" -F "caption=$CAPTION" | jq .
        ;;
    ...
    --sendaudiofile)
        ...
        local AUDIO=$2
        local CAPTION=$3
        curl -s "$API/sendAudio" -F "chat_id=$CHAT_ID" -F "audio=@\"$AUDIO\"" -F "caption=$CAPTION" | jq .
        ;;
